    ...
    <article className="glass-card p-3 flex flex-col gap-3">
      ...
      <div>
        <button
          ...
          title={item.itemWebUrl ? t('marketItemCard.openOnEbay') : t('marketItemCard.ebayLinkUnavailable')}
        >
          {item.title}
        </button>
      ...
      <div className="grid grid-cols-2 gap-2 text-xs">
        <div className="rounded-lg border border-slate-200 dark:border-slate-700 p-2">
          <p className="text-slate-500 dark:text-slate-300">{t('marketItemCard.itemPrice')}</p>
          <p className="font-semibold text-slate-900 dark:text-slate-100">{formatCurrency(item.priceValue)}</p>
        </div>
        <div className="rounded-lg border border-slate-200 dark:border-slate-700 p-2">
          <p className="text-slate-500 dark:text-slate-300">{t('marketItemCard.condition')}</p>
          <p className="font-semibold text-slate-900 dark:text-slate-100">{item.condition || t('marketItemCard.na')}</p>
        </div>
      </div>
      ...
      <div className="flex gap-2 mt-auto">
        <button type="button" onClick={() => onSelect(item)} className="btn-secondary flex-1 text-xs">
          ...
          {isSelected ? t('marketItemCard.selected') : t('marketItemCard.compare')}
        </button>
        <button type="button" onClick={() => onSearchTitle?.(item)} className="btn-secondary text-xs px-3" title={t('marketItemCard.searchTitle')}>
          <Search size={14} />
        </button>
        ...
        {amazonSearchUrl && (
          <a
            ...
            title={t('marketItemCard.searchOnAmazon')}
            aria-label={t('marketItemCard.searchOnAmazon')}
          >
