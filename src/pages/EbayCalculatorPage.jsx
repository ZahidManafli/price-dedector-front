          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <h1 className={`text-3xl font-bold ${isDark ? 'text-slate-100' : 'text-slate-900'}`}>
                {t('ebayCalculatorPage.title')}
              </h1>
              <p className={`mt-1 text-sm ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>
                {t('ebayCalculatorPage.subtitle')}
              </p>
            </div>
          </div>
        </div>
          ...
          >
            <div className="flex items-center justify-between gap-3 mb-4">
              <h2 className={`text-sm font-semibold ${isDark ? 'text-slate-100' : 'text-slate-900'}`}>
                {t('ebayCalculatorPage.inputPanelTitle')}
              </h2>
            </div>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-slate-500 mb-1">{t('ebayCalculatorPage.amazonCostLabel')}</label>
                  <input type="number" min="0" step="0.01" value={amazonCost} onChange={(e) => setAmazonCost(e.target.value)} className="input-base" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-slate-500 mb-1">{t('ebayCalculatorPage.ebayPriceLabel')}</label>
                  <input type="number" min="0" step="0.01" value={ebayPrice} onChange={(e) => setEbayPrice(e.target.value)} className="input-base" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-slate-500 mb-1">{t('ebayCalculatorPage.addRateLabel')}</label>
          ...
          <div className="space-y-4">
            <div className={`rounded-2xl border p-5 ${isDark ? 'bg-slate-950 border-slate-800' : 'bg-white border-slate-200'}`}>
              <h2 className={`text-sm font-semibold mb-4 ${isDark ? 'text-slate-100' : 'text-slate-900'}`}>
                {t('ebayCalculatorPage.liveResultsTitle')}
              </h2>

              <div className="space-y-3">
                <div className={`rounded-xl border p-4 ${isDark ? 'border-slate-800 bg-slate-900' : 'border-slate-200 bg-slate-50'}`}>
                  <p className="text-xs text-slate-500">{t('ebayCalculatorPage.estimatedProfitLabel')}</p>
                ...
                <div className={`rounded-xl border p-4 ${isDark ? 'border-slate-800 bg-slate-900' : 'border-slate-50 bg-slate-50'}`}>
                  ...
                  <p className={`mt-1 text-xs ${isDark ? 'text-slate-300' : 'text-slate-500'}`}>
                    {t('ebayCalculatorPage.netProfitLabel')}: {formatCurrency(parsed.netProfit)}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className={`rounded-2xl border p-5 ${isDark ? 'bg-slate-950 border-slate-800' : 'bg-white border-slate-200'}`}>
              <div className="flex items-center gap-2 mb-2">
                ...
                <p className={`text-sm font-semibold ${isDark ? 'text-slate-100' : 'text-slate-900'}`}>{t('ebayCalculatorPage.healthCheckTitle')}</p>
              </div>
              <p className="text-xs text-slate-500">
                {t('ebayCalculatorPage.denominatorLabel')}: <span className="font-semibold">{parsed.denominator.toFixed(4)}</span>
              </p>
              <div className="mt-2 text-xs text-slate-500 flex items-center gap-2">
                <TrendingUp size={14} />
                {t('ebayCalculatorPage.denominatorHint')}
              </div>
            </div>
          </div>
        </div>
