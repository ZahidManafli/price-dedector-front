...
function FloatingToolbar({ iframeRef }) {
  ...
  return (
    ...
    >
      {/* Undo / Redo */}
      <button className={btn(false)} onClick={handleUndo} title="Undo">↩</button>
      <button className={btn(false)} onClick={handleRedo} title="Redo">↪</button>
      ...
      <button className={btn(activeFormats.bold)} onClick={() => execCmd('bold')} title="Bold"><b>B</b></button>
      <button className={btn(activeFormats.italic)} onClick={() => execCmd('italic')} title="Italic"><i>I</i></button>
      <button className={btn(activeFormats.underline)} onClick={() => execCmd('underline')} title="Underline"><u>U</u></button>
      <button className={btn(activeFormats.strikeThrough)} onClick={() => execCmd('strikeThrough')} title="Strikethrough"><s>S</s></button>

      <div className="w-px h-5 bg-slate-300 mx-1" />

      {/* Alignment */}
      <button className={btn(activeFormats.justifyLeft)} onClick={() => execCmd('justifyLeft')} title="Align left">
      ...
      <button className={btn(activeFormats.justifyCenter)} onClick={() => execCmd('justifyCenter')} title="Align center">
      ...
      <button className={btn(activeFormats.justifyRight)} onClick={() => execCmd('justifyRight')} title="Align right">
      ...
      {/* Lists */}
      <button className={btn(activeFormats.insertUnorderedList)} onClick={() => execCmd('insertUnorderedList')} title="Bullet list">
      ...
      <button className={btn(activeFormats.insertOrderedList)} onClick={() => execCmd('insertOrderedList')} title="Numbered list">
      ...
      {/* Link */}
      <button className={btn(false)} onClick={handleLink} title="Insert link">
      ...
      {/* Image */}
      <button className={btn(false)} onClick={handleImage} title="Insert image">
      ...
      {/* Remove formatting */}
      <button className={btn(false)} onClick={() => execCmd('removeFormat')} title="Remove formatting">
      ...
      {showLinkInput && (
        <div className="flex items-center gap-1 ml-1">
          ...
          />
          <button className="text-xs px-2 py-1 bg-indigo-600 text-white rounded" onClick={applyLink}>OK</button>
          <button className="text-xs px-2 py-1 bg-slate-200 rounded" onClick={() => setShowLinkInput(false)}>✕</button>
        </div>
      )}
    </div>
  );
...
export default function DewisoPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setAlert({ type: 'error', message: err.response?.data?.error || 'Failed to load history' });
      } finally {
  ...
  const writeIframe = useCallback((html) => {
    ...
    doc.body.addEventListener('click', (e) => {
      ...
      if (!img) return;
      const choice = window.confirm('OK: enter image URL.\nCancel: upload file (not supported in iframe, use sidebar).');
      if (!choice) return;
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      setAlert({ type: 'success', message: `Uploaded ${items.length} image(s)` });
    } catch (err) {
  ...
  const saveTemplate = async () => {
    try {
      ...
      setAlert({ type: 'error', message: err.response?.data?.error || 'Save failed' });
    } finally {
      setSaving(false);
    }
  };
  ...
  const copyText = async (value) => {
    try {
      ...
      setAlert({ type: 'warning', message: 'Could not copy automatically' });
    }
  };
  ...
  return (
    <div className="page-shell">
      ...
      <div className="mb-4 flex items-center justify-between">
        <h1 className={`page-title ${isDark ? 'text-slate-100' : ''}`}>Dewiso Template Editor</h1>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-4 mt-4">
        ...
        <div className={`xl:col-span-4 rounded-2xl border p-4 space-y-4 ${isDark ? 'bg-slate-900/60 border-slate-700' : 'bg-white border-slate-200'}`}>
          ...
          <div>
            <p className={`text-sm font-semibold mb-2 ${isDark ? 'text-slate-100' : 'text-slate-900'}`}>
              Color <strong>Nav/Footer</strong>
            </p>
          ...
          <div>
            <p className={`text-sm font-semibold mb-2 ${isDark ? 'text-slate-100' : 'text-slate-900'}`}>
              Color <strong>Content Area</strong>
            </p>
            <div className="grid grid-cols-2 gap-2">
              <label className="text-xs text-slate-500">
                Background
          ...
          {uploadedImages.length > 0 && (
            <div>
              <p className={`text-sm font-semibold mb-2 ${isDark ? 'text-slate-100' : 'text-slate-900'}`}>Uploaded Images</p>
                ...
                {uploadedImages.map((img, idx) => (
                  <div key={idx} className={`rounded-lg border p-2 flex gap-2 items-start ${isDark ? 'border-slate-700 bg-slate-900' : 'border-slate-200 bg-slate-50'}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-semibold truncate">{img?.fileName || `Image ${idx + 1}`}</p>
                      {img?.ebayUrl && (
        ...
        <div className={`xl:col-span-8 rounded-2xl border overflow-hidden flex flex-col ${isDark ? 'bg-slate-900/60 border-slate-700' : 'bg-white border-slate-200'}`}>
          ...
          <div className={`px-4 py-2 border-b text-sm flex justify-between items-center ${isDark ? 'border-slate-700 text-slate-200' : 'border-slate-200 text-slate-700'}`}>
            <span className="font-medium">Live Preview — click any text to edit</span>
            <div className="flex gap-2">
              <button className="text-xs btn-secondary px-2 py-1" onClick={() => copyText(fullHtml)}>Copy HTML</button>
              <button className="text-xs btn-secondary px-2 py-1" onClick={downloadHtml}>Download HTML</button>
            </div>
          </div>
